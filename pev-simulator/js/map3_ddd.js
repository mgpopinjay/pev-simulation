...
$.get('id_counter.txt', function(data) {
    counter = data-1;
 }, 'text');
...
//  Value
    ...
    function test_fleet_sim() {
      ...
      // render(JSON.parse(localStorage.getItem('myData')));
      // })
...
function render(data) {
  document.getElementById('svgContainer').innerHTML = '<svg width="600" height="400"></svg>';
  ...
  var line = d3.line()
      ...
      .y(function(d) { return y(d.waitTime); });
  ...
  y.domain([
    d3.min(data, function(c) { return d3.min(c.values, function(d) { return d.waitTime; }); }),
    d3.max(data, function(c) { return d3.max(c.values, function(d) { return d.waitTime; }); })
  ]);
  ...
  temp.append("text")
      ...
      .attr("transform", function(d) { return "translate(" + x(d.value.time) + "," + y(d.value.waitTime) + ")"; })
      .attr("x", 3)
      .attr("dy", "0.35em")
...
function getData(){
  return new Promise(function(resolve, reject) {
    $.getJSON('../Variables/Variable_'+(ran_nums['num']--)+'.json',function(json){
        code = json.Code
        console.log(code);

    $.getJSON('../Results/trial_sim_hubway_data'+code+'.JSON', function(json) {
        // create an array of all durations for type 'Navigation'
         let fleet = json.fleet
         let durations = []
         _.each(fleet, function(car) {
           _.each(car.history, function(trip){
             let type = trip.type
             if (type === 'Navigation'){
         ...
         // sort durations in increasing order
         let sortedDurations = durations.sort();

         // group durations by intervals of 5
         let tempGroup = {};
         for (let i = 0; i < sortedDurations.length; i++) {
           let curr_group = Math.floor(sortedDurations[i] / 5);
           tempGroup[curr_group] = tempGroup[curr_group] || [];
           tempGroup[curr_group].push(sortedDurations[i]);
         }

         let groupedDurations = [];
         for (var key = 0; key < 11; key++) {
           if (tempGroup.hasOwnProperty(key)) {
             groupedDurations.push({
               time: key*5,
               waitTime: tempGroup[key].length
             });
           } else {
             groupedDurations.push({
               time: key*5,
         ...
         resolve({ id: "Trail"+(id++), values: groupedDurations });
